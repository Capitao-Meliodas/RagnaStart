...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	if (
		...
		(p->show_equip != cp->show_equip) || (p->queue_delay != cp->queue_delay) || (p->ranked_points != cp->ranked_points)
	)
	{	//Save status
		if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
			...
			"`queue_delay_timer`='%d', `ranked_points`='%f'"
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	struct item tmp_item; // [CreativeSD]: Restock System
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`queue_delay_timer`, `ranked_points`"
	...
	||  SQL_ERROR == SqlStmt_BindColumn(stmt, 62, SQLDT_INT,    &p->queue_delay, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 63, SQLDT_FLOAT,  &p->ranked_points, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
	...
	while (SQL_SUCCESS == SqlStmt_NextRow(stmt)) {
		memcpy(&p->restock[i], &tmp_item, sizeof(tmp_item));
	}
