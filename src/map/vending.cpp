 ...
 *------------------------------------------*/
// Get a free block.
static int organized_shops_get_block(int m) {
	int i;
...
static void organized_shops_xy(int x1, int y1, int x2, int y2, int type, int *x, int *y)
{
	if( type == 0 || type == 2 )
		*x = x1+x2;
...
int8 vending_openvending(struct map_session_data* sd, const char* message, const uint8* data, int count, struct s_autotrader *at)
{
	...
	// [CreativeSD]: Organized Shops
	if( map[sd->bl.m].flag.organized_shops ) {
