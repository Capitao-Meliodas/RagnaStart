...
static int clif_setlevel(struct block_list* bl) {
	...
	if (bl->type == BL_PC) {
		...
		if (sd->organized_shops.flag && sd->vending)
			return 1;
	}
...
static int clif_set_unit_idle(struct block_list* bl, unsigned char* buffer, bool spawn)
{
	...
	{	//The hell, why flags work like this?
		WBUFW(buf,22) = status_get_emblem_id(bl,false);
		WBUFW(buf,24) = GetWord(status_get_guild_id(bl,false), 1);
		WBUFW(buf,26) = GetWord(status_get_guild_id(bl,false), 0);
	}
	...
	buf = WBUFP(buffer,offset);
#endif
	WBUFL(buf,34) = status_get_guild_id(bl,false);
	WBUFW(buf,38) = status_get_emblem_id(bl,false);
...
static int clif_set_unit_walking(struct block_list* bl, struct unit_data* ud, unsigned char* buffer)
{
	...
	buf = WBUFP(buffer,offset);
#endif
	WBUFL(buf,38) = status_get_guild_id(bl,false);
	WBUFW(buf,42) = status_get_emblem_id(bl,false);
...
void clif_selllist(struct map_session_data *sd)
{
	...
	for( i = 0; i < MAX_INVENTORY; i++ )
	{
		if( sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory_data[i] )
		{
			...
			if( sd->inventory.u.items_inventory[i].card[0]==CARD0_CREATE && MakeDWord(sd->inventory.u.items_inventory[i].card[2],sd->inventory.u.items_inventory[i].card[3])== battle_config.stuff_bg_reserverd_id && !(battle_config.stuff_enable_sell&0x1) )
				continue;

			if( sd->inventory.u.items_inventory[i].card[0]==CARD0_CREATE && MakeDWord(sd->inventory.u.items_inventory[i].card[2],sd->inventory.u.items_inventory[i].card[3])== battle_config.stuff_gvg_reserverd_id && !(battle_config.stuff_enable_sell&0x2) )
				continue;

			if( sd->inventory.u.items_inventory[i].card[0]==CARD0_CREATE && MakeDWord(sd->inventory.u.items_inventory[i].card[2],sd->inventory.u.items_inventory[i].card[3])== battle_config.stuff_pvp_reserverd_id && !(battle_config.stuff_enable_sell&0x4) )
				continue;
#endif
...
static void clif_getareachar_pc(struct map_session_data* sd,struct map_session_data* dstsd)
{
	...
	} else if( dstsd->state.vending && (!dstsd->organized_shops.flag || battle_config.organized_shops_ballon) )
		clif_showvendingboard(&dstsd->bl,dstsd->message,sd->fd); // [CreativeSD]: Organized Shops Modify
...
#ifdef RANKED_SYSTEM_ENABLE
			if( tsd->ranked.current_rank ) // [CreativeSD]: Ranked System
				ranked_show_aura(bl);
#endif
		}
		break;
	...
	case BL_MOB:
		{
			...
			if (md->bg_id && md->bg_id != sd->bg_id && map[md->bl.m].flag.battleground)
...
void clif_guild_basicinfo(struct map_session_data *sd) {
	...
	WFIFOL(fd,42)=status_get_emblem_id(&sd->bl,true);
...
void clif_guild_emblem(struct map_session_data *sd,struct guild *g)
{
	...
	if (sd->bg_id)	{
		...
		if (bg != NULL && bg->g != NULL)
			g = bg->g; // Change emblem to Battleground
	}
#endif
...
void clif_guild_emblem_area(struct block_list* bl)
{
	...
	WBUFW(buf,0) = 0x1b4;
	WBUFL(buf,2) = bl->id;
	WBUFL(buf,6) = status_get_guild_id(bl,false);
	WBUFW(buf,10) = status_get_emblem_id(bl,false);
...
#ifdef RANKED_SYSTEM_ENABLE
			// [CreativeSD]: Ranked System
			// always before BG Warfare
			if( !sd->ranked.disable && sd->ranked.current_rank && ranked_show_map(sd->bl.m) && (sd->ranked.display_pos == RANK_SHOW || (!sd->bg_id && !sd->status.guild_id)) )
			{
				struct ranked_data *rank = ranked_search(sd->ranked.current_rank);
				if( rank != NULL && rank->g != NULL )
				{
					if( sd->fakename[0] )
					{
						memcpy(WBUFP(buf,6), sd->fakename, NAME_LENGTH);
						WBUFB(buf,30) = 0;
					}
					else {
					...
					memcpy(WBUFP(buf,54), "Rank # ",NAME_LENGTH);
					memcpy(WBUFP(buf,78), rank->g->name, NAME_LENGTH);
				}
#if PACKETVER >= 20150513
...
#ifdef BG_WARFARE_ENABLE
			// [CreativeSD]: Battleground Warfare
			if( battle_config.bg_name_position && sd->bg_id )
			{
				struct battleground_data *bg = bg_team_search(sd->bg_id);
				if( bg != NULL && bg->g != NULL )
				{
					int position = sd->bg_position;
					if( position >= MAX_BG_POSITION )
						position = MAX_BG_POSITION - 1;

					safestrncpy(WBUFCP(buf, 6), sd->status.name, NAME_LENGTH);
					WBUFB(buf, 30) = 0;
					safestrncpy(WBUFCP(buf,54), bg->g->name,NAME_LENGTH);
					safestrncpy(WBUFCP(buf,78), bg->g->position[position].name, NAME_LENGTH);
#if PACKETVER >= 20150513
...
#ifdef BG_WARFARE_ENABLE
			// [CreativeSD]: Battleground Warfare
			if (battle_config.bg_name_position && md->bg_id && (bg = bg_team_search(md->bg_id)) != NULL && md->desc[0])
			{
#if PACKETVER >= 20150513
				WBUFW(buf, 0) = cmd = 0xa30;
#else
				...
				memcpy(WBUFP(buf, 78), bg->g->name, NAME_LENGTH);
			}
			else
#endif
...
#ifdef BG_WARFARE_ENABLE
	else if( map_flag_gvg(sd->bl.m) || map[sd->bl.m].flag.battleground )
#else
...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if (sd->bg_id) {
		...
		if (bg != NULL && bg->g != NULL)
		{
			guild_guildaura_refresh(sd, GD_LEADERSHIP, guild_checkskill(bg->g, GD_LEADERSHIP));
			guild_guildaura_refresh(sd, GD_GLORYWOUNDS, guild_checkskill(bg->g, GD_GLORYWOUNDS));
			guild_guildaura_refresh(sd, GD_SOULCOLD, guild_checkskill(bg->g, GD_SOULCOLD));
			guild_guildaura_refresh(sd, GD_HAWKEYES, guild_checkskill(bg->g, GD_HAWKEYES));
		}
	}
#endif
	...
	if( battle_config.queue_only_towns && sd->queue_id && !queue_check_town(sd->bl.m) )
		queue_leave(sd,1);
#endif
...
#ifdef RANKED_SYSTEM_ENABLE
	// [CreativeSD]: Ranked System
	if( !sd->ranked.disable || (!map[sd->bl.m].flag.town && sd->ranked.display_timer == INVALID_TIMER) ) {
		int rk_chg_timer = sd->status.guild_id ? ranked_config.show_timer : 1;
		float points = ranked_config.show_mode ? sd->ranked.account_points : sd->status.ranked_points;
		
		sd->ranked.disable = 0;
		sd->ranked.current_rank = ranked_get_rank(points);
		sd->ranked.display_timer = add_timer(gettick()+rk_chg_timer,ranked_change_emblem,sd->bl.id,0);
	}

	ranked_show_aura(&sd->bl);
	if (ranked_check_zone(sd->bl.m))
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, RANKED_MSGTXT+5), false, SELF);
#endif
...
void clif_parse_WalkToXY(int fd, struct map_session_data *sd)
{
	...
	{
		delete_timer(sd->bg_afk_timer, bg_report_afk);
		...
		case BL_PC:
			{
				...
				if( tsd->organized_shops.flag )
					vending_vendinglistreq(sd,tsd->status.account_id);
				else
...
void clif_parse_UseSkillToId(int fd, struct map_session_data *sd)
{
	...
	if( SKILL_CHK_GUILD(skill_id) ) {
		...
		struct battleground_data *bg = NULL;
#ifdef BG_WARFARE_ENABLE
		if( sd->bg_id && (bg = bg_team_search(sd->bg_id)) != NULL && bg != NULL )
			skill_lv = guild_checkskill(bg->g, skill_id);
		else
#endif
		if( sd->state.gmaster_flag )
...
#ifdef BG_WARFARE_ENABLE
	// [CreativeSD]: Battleground Warfare
	if( guild_id >= (SHRT_MAX-1) )
	{
		struct battleground_data *bg = bg_team_search(guild_id);
		if( bg != NULL && bg->g != NULL )
			g = bg->g;
	}
#endif

	if( g != NULL || (g = guild_search(guild_id)) != NULL )
...
void clif_parse_GuildExpulsion(int fd,struct map_session_data *sd){
	...
	if( sd->bg_id ) {
		...
		int char_id = RFIFOL(fd,info->pos[2]);

		if( (pl_sd = map_charid2sd(char_id)) != NULL && pl_sd->bg_id && pl_sd->bg_id == sd->bg_id )
		{
			bg_kick_player(sd,pl_sd, RFIFOCP(fd,info->pos[3]));
			return;
		}
	}
#endif
...
#ifdef BG_WARFARE_ENABLE
	// [CreativeSD]: Battleground Warfare
	if( !(battle_config.bg_enable_cross_swords&0x02) )
		return;
#endif
 ...
 * CreativeSD: Battleground Warfare
 *------------------------------------------*/
#include "clif_battleground_func.inc"
#endif
