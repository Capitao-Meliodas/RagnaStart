...
#ifdef BG_WARFARE_ENABLE
	CELL_NOBATTLEGROUND = 15,	// [CreativeSD] No Battleground Warfare vs in cell.
#endif
...
#ifdef BG_WARFARE_ENABLE
	CELL_CHKNOBATTLEGROUND = 21,	// [CreativeSD] No Battleground Warfare vs in cell.
#endif
...
struct mapcell
{
	...
	unsigned char
		...
		maelstrom : 1,
#ifdef BG_WARFARE_ENABLE
		nobattleground : 1, // [CreativeSD] No Battleground Warfare vs in cell. 
#endif
		icewall : 1;
...
struct map_data {
...
#endif
#ifdef BG_WARFARE_ENABLE
		unsigned nobgskillcall : 1; // [CreativeSD]: Battleground Warfare
		unsigned nobgrespawn : 1;
#endif
#ifdef RANKED_SYSTEM_ENABLE
		unsigned ranked : 1; // [CreativeSD]: Ranked System
		unsigned noranked : 1;
#endif
#ifdef STUFF_ITEMS_ENABLE
		// [CreativeSD]: Stuff System
		unsigned bg_consume : 1; // allows using Battleground's items
		unsigned gvg_consume : 1; // allows using GvG's items
		unsigned pvp_consume : 1; // allows using PvP's items
#endif
		unsigned organized_shops : 1; // [CreativeSD]: Organized Shops
		unsigned test_emperium : 1; // [CreativeSD]: Test Emperium
	} flag;
#ifdef RANKED_SYSTEM_ENABLE
	// [CreativeSD]: Ranked System (Points in Maps)
	struct ranked_flag {
	...
	struct {
		...
		int count;
		int class_;
	} organized_shops[MAX_OSHOPS_BLOCK];
	struct point save;
