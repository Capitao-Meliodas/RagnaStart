...
struct mob_data {
	...
	char desc[NAME_LENGTH]; // [CreativeSD]: Battleground Warfare (desc)
#endif
...
int mob_spawn_bg_sub(const char* mapname, int16 x, int16 y, const char* mobname, int mob_id, const char* event, unsigned int bg_id, const char* info);
#ifdef BG_WARFARE_ENABLE
#define mob_spawn_bg(mapname, x, y, mobname, mob_id, ev, bg_id, info) mob_spawn_bg_sub(mapname, x, y, mobname, mob_id, ev, bg_id, info)
#else
#define mob_spawn_bg(mapname, x, y, mobname, mob_id, ev, bg_id) mob_spawn_bg_sub(mapname, x, y, mobname, mob_id, ev, bg_id, "")
#endif
