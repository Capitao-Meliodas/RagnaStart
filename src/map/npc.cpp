...
/**
 * [CreativeSD]: Item Shop Buy Event
 */
static int npc_cashshop_buylist_sub(struct map_session_data* sd, unsigned short itemreq, const char *reg, int discount, int n, unsigned short* item_list, int point, struct npc_data* nd) {
...
#ifdef STUFF_ITEMS_ENABLE
	// [CreativeSD]: Item/Point Shop Buy Item Event
	if( nd->master_nd ) {
		if( nd->subtype == NPCTYPE_ITEMSHOP ) {
			int cost[2] = { 0,0 };
			npc_shop_currency_type(sd, nd, cost, false);
			npc_cashshop_buylist_sub(sd, nd->u.shop.itemshop_nameid, "", nd->u.shop.discount, count, item_list, vt, nd->master_nd);
			return ERROR_TYPE_NONE;
		}
		else if( nd->subtype == NPCTYPE_POINTSHOP ) {
			int cost[2] = { 0,0 };
			npc_shop_currency_type(sd, nd, cost, false);
			npc_cashshop_buylist_sub(sd, 0, nd->u.shop.pointshop_str, nd->u.shop.discount, count, item_list, vt, nd->master_nd);
			return ERROR_TYPE_NONE;
		}
	}
#endif
...
static const char* npc_parse_mapflag(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	else if (!strcmpi(w3, "nobgskillcall"))
		map[m].flag.nobgskillcall=state;
	else if (!strcmpi(w3, "nobgrespawn"))
		map[m].flag.nobgrespawn=state;
#endif
#ifdef RANKED_SYSTEM_ENABLE
	else if (!strcmpi(w3,"ranked"))
		map[m].flag.ranked=state;
	else if (!strcmpi(w3,"noranked"))
		map[m].flag.noranked = state;
	else if (!strcmpi(w3,"ranked_kill_points_gain"))
	{
		if( state )
		{
			float state2;
			if( sscanf(w4, "%f", &state2) == 1 )
				map[m].ranked_flag.kill_points_gain = state2;
			else
				map[m].ranked_flag.kill_points_gain = ranked_config.kill_points_gain;
		}
		else
			map[m].ranked_flag.kill_points_gain = ranked_config.kill_points_gain;
	}
	else if (!strcmpi(w3,"ranked_kill_points_drop"))
	{
		if( state )
		{
			float state2;
			if( sscanf(w4, "%f", &state2) == 1 )
				map[m].ranked_flag.kill_points_drop = state2;
			else
				map[m].ranked_flag.kill_points_drop = ranked_config.kill_points_drop;
		}
		else
			map[m].ranked_flag.kill_points_drop = ranked_config.kill_points_drop;
	}
	else if (!strcmpi(w3,"ranked_support_points"))
	{
		if( state )
		{
			float state2;
			if( sscanf(w4, "%f", &state2) == 1 )
				map[m].ranked_flag.support_points = state2;
			else
				map[m].ranked_flag.support_points = ranked_config.support_points;
		}
		else
			map[m].ranked_flag.support_points = ranked_config.support_points;
	}
	...
	else if (!strcmpi(w3, "gvg_consume"))
		map[m].flag.gvg_consume = state;
	else if (!strcmpi(w3, "pvp_consume"))
		map[m].flag.pvp_consume = state;
#endif
	/*===========================================
	 * [CreativeSD]: Organized Shops
	 *------------------------------------------*/
	else if (!strcmpi(w3,"organized_shops")) {
		int i, class_, ix, iy, d, cell, a, max;
		if( sscanf(w4, "%d,%d,%d,%d,%d,%d,%d,%d", &i, &class_, &ix, &iy, &d, &a, &cell, &max) != 8 )
			ShowWarning("npc_parse_mapflag: Number of invalid arguments in mapflag! Skip Vend Bulleting Board flag from %s (file '%s', line '%d').\n", map[m].name, filepath, strline(buffer,start-buffer));
		else {
			if( class_ > 0 && !npcdb_checkid(class_) )
				ShowWarning("npc_parse_mapflag: You must use a valid npc viewid in organized shops mapflag! Skip Vend Bulleting Board flag from %s (file '%s', line '%d').\n", map[m].name, filepath, strline(buffer,start-buffer));
			else if( ix <= 0 || iy <= 0 )
				ShowWarning("npc_parse_mapflag: You can not use random coordinates in organized shops mapflag! Skip Vend Bulleting Board flag from %s (file '%s', line '%d').\n", map[m].name, filepath, strline(buffer,start-buffer));
			else if( d < 0 || d > 3 )
				ShowWarning("npc_parse_mapflag: You must use a valid direction in organized shops mapflag! Skip Vend Bulleting Board flag from %s (file '%s', line '%d').\n", map[m].name, filepath, strline(buffer,start-buffer));
			else if( a <= 0 )
				ShowWarning("npc_parse_mapflag: You must set a quantity for column in organized shops mapflags! Skip Vend Bulleting Board flag from %s (file '%s', line '%d').\n", map[m].name, filepath, strline(buffer,start-buffer));
			else if( cell <= 0 )
				ShowWarning("npc_parse_mapflag: You must set a cell for column in organized shops mapflags! Skip Vend Bulleting Board flag from %s (file '%s', line '%d').\n", map[m].name, filepath, strline(buffer,start-buffer));
			else if( max <= 0 )
				ShowWarning("npc_parse_mapflag: You should set the sales limit on the organized shops mapflag! Skip Vend Bulleting Board flag from %s (file '%s', line '%d').\n", map[m].name, filepath, strline(buffer,start-buffer));
			else if( i < MAX_OSHOPS_BLOCK ) {
				map[m].flag.organized_shops = state;
				map[m].organized_shops[i].class_ = class_;
				map[m].organized_shops[i].x = ix;
				map[m].organized_shops[i].y = iy;
				map[m].organized_shops[i].dir = d;
				map[m].organized_shops[i].amount = a;
				map[m].organized_shops[i].cell = cell;
				map[m].organized_shops[i].max = max;
			}
			else
				ShowWarning("npc_parse_mapflag: You have exceeded the limit of organized shops blocks! Skip Vend Bulleting Board flag from %s (file '%s', line '%d').\n", map[m].name, filepath, strline(buffer,start-buffer));
		}
	}
	// [CreativeSD]: Test Emperium
	else if (!strcmpi(w3, "test_emperium"))
		map[m].flag.test_emperium=state;
	else
