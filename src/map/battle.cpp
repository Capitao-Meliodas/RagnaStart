...
bool battle_can_hit_gvg_target(struct block_list *src,struct block_list *bl,uint16 skill_id,int flag)
{
	...
	if(md && md->guardian_data) {
		...
		if( src->type != BL_MOB || mob_is_clone( ((struct mob_data*)src)->mob_id ) ){
			struct guild *g = src->type == BL_PC ? ((TBL_PC *)src)->guild : guild_search(status_get_guild_id(src,true));
...
void battle_consume_ammo(struct map_session_data*sd, int skill, int lv)
{
	...
	if (sd->equip_index[EQI_AMMO] >= 0) //Qty check should have been done in skill_check_condition
	{
		...
		// [CreativeSD]: Battleground System
		if (sd->bg_id && map[sd->bl.m].flag.battleground)
			increment_limit(sd->bg_score.ammos, qty, USHRT_MAX);
#endif
	}
...
#ifdef BG_WARFARE_ENABLE
			if( !map_flag_gvg(m) && md->guardian_data && md->guardian_data->guild_id && (!map[m].flag.battleground) )
#else
	...
	{ //Check rivalry settings.
		...
		if( flag&(BCT_PARTY|BCT_ENEMY) )
		{
			...
			if( s_party && s_party == status_get_party_id(t_bl) && !(map[m].flag.pvp && map[m].flag.pvp_noparty) && !(map_flag_gvg(m) && map[m].flag.gvg_noparty) && (!map[m].flag.battleground || sbg_id == tbg_id) )
				state |= BCT_PARTY;
#ifdef BG_WARFARE_ENABLE
			else if (map[m].flag.battleground && map_getcell(s_bl->m, s_bl->x, s_bl->y, CELL_CHKNOBATTLEGROUND))
		...
		{
			int s_guild = status_get_guild_id(s_bl,true);
			int t_guild = status_get_guild_id(t_bl,true);
#ifdef BG_WARFARE_ENABLE
			// [CreativeSD]: Battleground Warfare
			if (map[m].flag.battleground && (sbg_id || tbg_id))
			{
				if (sbg_id == tbg_id)
					state |= BCT_GUILD;
				else
...
// rAthena
	 ...
	 *---------------------------------------------------
	 */
	{ "stuff_bg_reserverd_id",              &battle_config.stuff_bg_reserverd_id,          165100,  0,      INT_MAX,        },
	{ "stuff_gvg_reserverd_id",             &battle_config.stuff_gvg_reserverd_id,         165101,  0,      INT_MAX,        },
	{ "stuff_pvp_reserverd_id",             &battle_config.stuff_pvp_reserverd_id,         165102,  0,      INT_MAX,        },
