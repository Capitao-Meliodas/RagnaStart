...
uint8 pc_isequip(struct map_session_data *sd,int n)
{
	...
	// [CreativeSD]: Stuff Items equip restricted
	if( sd->inventory.u.items_inventory[n].card[0]==CARD0_CREATE &&
	    MakeDWord(sd->inventory.u.items_inventory[n].card[2],sd->inventory.u.items_inventory[n].card[3])== battle_config.stuff_bg_reserverd_id &&
	    !map[sd->bl.m].flag.bg_consume
	)
		return ITEM_EQUIP_ACK_FAIL;

	if( sd->inventory.u.items_inventory[n].card[0]==CARD0_CREATE &&
	    MakeDWord(sd->inventory.u.items_inventory[n].card[2],sd->inventory.u.items_inventory[n].card[3])== battle_config.stuff_gvg_reserverd_id &&
	    !map[sd->bl.m].flag.gvg_consume
	)
		return ITEM_EQUIP_ACK_FAIL;

	if( sd->inventory.u.items_inventory[n].card[0]==CARD0_CREATE &&
	    MakeDWord(sd->inventory.u.items_inventory[n].card[2],sd->inventory.u.items_inventory[n].card[3])== battle_config.stuff_pvp_reserverd_id &&
	    !map[sd->bl.m].flag.pvp_consume
	)
		return ITEM_EQUIP_ACK_FAIL;
#endif
...
void pc_reg_received(struct map_session_data *sd)
{
	...
	{
		const char *rank_points = pc_readaccountregstr(sd, add_str("#RANKED_ACCOUNT_POINTS$"));
		float last_points = ranked_get_last_points();
		sd->ranked.account_points = (float)(rank_points == NULL ?0 : atof(rank_points));
		if (sd->ranked.account_points > last_points)
			sd->ranked.account_points = last_points;
		if (sd->status.ranked_points > last_points)
			sd->status.ranked_points = last_points;
	}
#endif
}
...
// CreativeSD: Restock System
static int pc_restock_item(struct map_session_data *sd, int itemid)
{
	int i, amount = 0;
	for (i = 0; i < MAX_RESTOCK; i++)
	{
		if (sd->status.restock[i].nameid == itemid)
		{
...
static void pc_restock_getstorage(struct map_session_data *sd, int itemid, int amount)
{
	int i;
	for (i = 0; i < MAX_STORAGE; i++)
	{
		if (sd->storage.u.items_storage[i].nameid == itemid)
		{
...
{
	int restock_nameid = 0, restock_amount = 0; // [CreativeSD]: Restock System
	...
	if( sd->inventory.u.items_inventory[n].amount <= 0 ){
		if ( !((!battle_config.restock_castle && map[sd->bl.m].flag.gvg_castle && agit_flag) || (!battle_config.restock_pvp && map[sd->bl.m].flag.pvp) || (!battle_config.restock_gvg && map[sd->bl.m].flag.gvg)) )
		{
			restock_nameid = sd->inventory.u.items_inventory[n].nameid;
			restock_amount = pc_restock_item(sd, sd->inventory.u.items_inventory[n].nameid);
		}
		...
		if ( restock_nameid && restock_amount )
			pc_restock_getstorage(sd, restock_nameid, restock_amount);
	}
	if(!(type&1))
...
unsigned char pc_cart_additem(struct map_session_data *sd,struct item *item,int amount,e_log_pick_type log_type)
{
	...
	// [CreativeSD]: Stuff Items
	// BG Stuff
	if( item->card[0]==CARD0_CREATE && MakeDWord(item->card[2],item->card[3])==battle_config.stuff_bg_reserverd_id && !(battle_config.stuff_enable_cart&0x01) )
	{
		clif_displaymessage (sd->fd, msg_txt(sd,264));
		return 1;
	}
	...
	// GvG Stuff
	if( item->card[0]==CARD0_CREATE && MakeDWord(item->card[2],item->card[3])==battle_config.stuff_gvg_reserverd_id && !(battle_config.stuff_enable_cart&0x02) )
	{
		clif_displaymessage (sd->fd, msg_txt(sd,264));
		return 1;
	}
	...
	// PvP Stuff
	if( item->card[0]==CARD0_CREATE && MakeDWord(item->card[2],item->card[3])==battle_config.stuff_pvp_reserverd_id && !(battle_config.stuff_enable_cart&0x04) )
	{
		clif_displaymessage (sd->fd, msg_txt(sd,264));
		return 1;
	}
...
#ifdef BG_WARFARE_ENABLE
		// [CreativeSD]: Battleground Warfrare
		if( sd->bg_id ) {
...
int pc_dead(struct map_session_data *sd,struct block_list *src)
{
	...
	if (src && src->type == BL_PC) {
		...
		{
			float points_gain = map[sd->bl.m].ranked_flag.kill_points_gain > 0 ? map[sd->bl.m].ranked_flag.kill_points_gain : ranked_config.kill_points_gain,
				  points_drop = map[sd->bl.m].ranked_flag.kill_points_drop > 0 ? map[sd->bl.m].ranked_flag.kill_points_drop : ranked_config.kill_points_drop;
			ranked_update_rank(ssd,points_gain,RANK_BOTH,RANK_P_KILL);
			ranked_update_rank(sd,-(points_drop),RANK_BOTH,RANK_P_KILL);
		}
#endif
...
bool pc_setcart(struct map_session_data *sd,int type) {
	...
	if( type )
		sd->organized_shops.cart_option = type;
...
#ifdef STUFF_ITEMS_ENABLE
	// [CreativeSD]: Stuff Item
	// BG Stuff
	if( item->card[0]==CARD0_CREATE && MakeDWord(item->card[2],item->card[3])==battle_config.stuff_bg_reserverd_id && !(battle_config.stuff_enable_drop&0x01) )
		return false;
	// GvG Stuff
	if( item->card[0]==CARD0_CREATE && MakeDWord(item->card[2],item->card[3])==battle_config.stuff_gvg_reserverd_id && !(battle_config.stuff_enable_drop&0x02) )
		return false;
	// PvP Stuff
	if( item->card[0]==CARD0_CREATE && MakeDWord(item->card[2],item->card[3])==battle_config.stuff_pvp_reserverd_id && !(battle_config.stuff_enable_drop&0x04) )
		return false;
#endif
