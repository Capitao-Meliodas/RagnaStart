...
#endif
#ifdef RANKED_SYSTEM_ENABLE
			case MF_RANKED:             script_pushint(st,map[m].flag.ranked); break;
			case MF_NORANKED:           script_pushint(st,map[m].flag.noranked); break;
#endif
#ifdef STUFF_ITEMS_ENABLE
			case MF_BG_CONSUME:		script_pushint(st,map[m].flag.bg_consume); break;
			case MF_GVG_CONSUME:	script_pushint(st, map[m].flag.gvg_consume); break;
			case MF_PVP_CONSUME:	script_pushint(st, map[m].flag.pvp_consume); break;
#endif
		}
	}
	return SCRIPT_CMD_SUCCESS;
...
#endif
#ifdef RANKED_SYSTEM_ENABLE
			case MF_RANKED:             map[m].flag.ranked=1; break;
			case MF_NORANKED:           map[m].flag.noranked=1; break;
#endif
#ifdef STUFF_ITEMS_ENABLE
			case MF_BG_CONSUME:			map[m].flag.bg_consume = 1; break;
			case MF_GVG_CONSUME:		map[m].flag.gvg_consume = 1; break;
			case MF_PVP_CONSUME:		map[m].flag.pvp_consume = 1; break;
#endif
		}
	}
	return SCRIPT_CMD_SUCCESS;
...
#endif
#ifdef RANKED_SYSTEM_ENABLE
			case MF_RANKED:             map[m].flag.ranked=0; break;
			case MF_NORANKED:           map[m].flag.noranked=0; break;
#endif
#ifdef STUFF_ITEMS_ENABLE
			case MF_BG_CONSUME:			map[m].flag.bg_consume = 0; break;
			case MF_GVG_CONSUME:		map[m].flag.gvg_consume = 0; break;
			case MF_PVP_CONSUME:		map[m].flag.pvp_consume = 0; break;
#endif
		}
	}
	return SCRIPT_CMD_SUCCESS;
...
BUILDIN_FUNC(waitingroom2bg)
{
...
#ifdef BG_WARFARE_ENABLE
	if( script_hasdata(st,12) )
		nd = npc_name2id(script_getstr(st,12));
#else
	if (script_hasdata(st, 7))
	...
	if(script_hasdata(st,9))
		ev = script_getstr(st,9); // Logout Event
	if(script_hasdata(st,10))
		dev = script_getstr(st,10); // Die Event
	if(script_hasdata(st,11))
		wev = script_getstr(st,11); // Wo Event
#else
	x = script_getnum(st,3);
	y = script_getnum(st,4);
	if(script_hasdata(st,5))
...
#ifdef BG_WARFARE_ENABLE
	check_event(st, wev);
	bg_id = bg_create(bg_id, mapindex, x, y, rx, ry, army, ev, dev, wev);
#else
	bg_id = bg_create(mapindex, x, y, ev, dev);
#endif
	...
	for (i = 0; i < cd->users; i++) { // Only add those who are in the chat room
		struct map_session_data *sd;
		if( (sd = cd->usersd[i]) != NULL && bg_team_join(bg_id, sd, 0) ){
...
BUILDIN_FUNC(waitingroom2bg_single)
{
	...
	if( bg_team_join(bg_id, sd, 0) && pc_setpos(sd, mapindex, x, y, CLR_TELEPORT) == SETPOS_OK)
	{
		script_pushint(st, true);
	}
	else
...
#ifdef BG_WARFARE_ENABLE
	script_pushint(st, mob_spawn_bg(mapname, x, y, str, class_, evt, bg_id, info));
#else
...
BUILDIN_FUNC(bg_get_data)
{
	...
	switch( type )
	{
		...
		case 1:
#ifdef BG_WARFARE_ENABLE
			script_pushint(st, bg->army); break; // 0: Guill/1: Croix
		case 2:
			script_pushint(st, bg->master_id); break;
		case 3:
			{
				struct map_session_data *sd = map_charid2sd(bg->master_id);
				if (sd != NULL)
					script_pushstrcopy(st, sd->status.name);
				else
			...
			break;
		case 4:
			script_pushstrcopy(st, bg->g->name);
			break;
		case 5:
#endif
 ...
 *------------------------------------------*/
#include "battleground_scriptfunc.inc"
#endif
 ...
 *------------------------------------------*/
#include "script_ranked_func.inc"
#endif

/// [CreativeSD]: Hunting Arena/Novice vs Zombie Commands/Wot
///
...
BUILDIN_FUNC(oshops_getinfo)
{
	...
	str=script_getstr(st,2);
	if( (m=map_mapname2mapid(str)) < 0 || !map[m].flag.organized_shops )
	{
		script_pushint(st,0);
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(oshops_remove)
{
	...
	if( (m=map_mapname2mapid(str)) < 0 )
		...
		map[m].flag.organized_shops = flag ? 1 : 0;
		script_pushint(st,1);
		return SCRIPT_CMD_SUCCESS;
	}
	script_pushint(st,0);
...
{
	struct Sql* handle = qsmysql_handle;
	TBL_PC *sd;
	struct script_data *data;
	struct item_data* id = NULL;
	int i = 0, amount;
	...
	{
		if (sd->status.restock[i].nameid == item_id)
		{
			flag = 1;
			break;
		}
	...
	{
		// Update Restock Item.
		if (SQL_ERROR == Sql_Query(handle, "UPDATE `restock` SET `amount`='%d' WHERE `char_id`='%d' AND `nameid`='%d'", amount, sd->status.char_id, item_id))
		{
			Sql_ShowDebug(handle);
			script_pushint(st, 0);
			return SCRIPT_CMD_SUCCESS;
		...
		// New Restock Item.
		if (SQL_ERROR == Sql_Query(handle, "INSERT INTO `restock` (`char_id`, `nameid`, `amount`) VALUES (%d, %d, %d)", sd->status.char_id, item_id, amount))
		{
			Sql_ShowDebug(handle);
			script_pushint(st, 0);
			return SCRIPT_CMD_SUCCESS;
		...
		sd->status.restock[i].nameid = item_id;
		sd->status.restock[i].amount = amount;
	}
...
BUILDIN_FUNC(delrestock)
{
	...
	for (i = 0; i < MAX_RESTOCK; i++)
	{
		...
		{
			// Update Restock Item.
			if (SQL_ERROR == Sql_Query(handle, "DELETE FROM `restock` WHERE `char_id`='%d' AND `nameid`='%d'", sd->status.char_id, item_id))
			{
				Sql_ShowDebug(handle);
				script_pushint(st, 0);
				return true;
			...
			memset(&sd->status.restock[i], 0, sizeof(sd->status.restock[0]));
			flag = true;
			break;
		}
	}
...
/// [CreativeSD]: bindclock "<hour>:<minute>", "<npc_event::label>";
...
/// [CreativeSD]: unbindclock "<hour>:<minute>", "<npc_event::label>";
...
// <--- [zBuffer] List of unit control commands
	...
	// BattleGround
#ifdef BG_WARFARE_ENABLE
	BUILDIN_DEF(waitingroom2bg,"isiii??????"),
#else
...
#ifdef QUEUE_SYSTEM_ENABLE
	// [CrativeSD] Queue System
#include "queue_scriptdef.inc"
#endif

#ifdef BG_WARFARE_ENABLE
	// [CreativeSD] Battleground Warfare
#include "battleground_scriptdef.inc"
#endif

#ifdef RANKED_SYSTEM_ENABLE
	// [CreativeSD]: Ranked System
#include "script_ranked_def.inc"
#endif
