...
#ifdef BG_WARFARE_ENABLE
		if (!gc && !map[md->bl.m].flag.battleground && !map[md->bl.m].flag.test_emperium)
			ShowError("status_calc_mob: No castle or battleground set at map %s\n", map[md->bl.m].name);
		else if (map[md->bl.m].flag.battleground && md->mob_id == MOBID_EMPERIUM) {
#ifdef RENEWAL
			status->max_hp += 50 * 100;
			status->max_sp += 70 * 100;
#else
			status->max_hp += 1000 * 100;
			status->max_sp += 200 * 100;
#endif
			status->hp = status->max_hp;
			status->sp = status->max_sp;
			status->def += (100+2)/3;
...
#else
		if (!gc && !map[md->bl.m].flag.test_emperium)
			ShowError("status_calc_mob: No castle set at map %s\n", map[md->bl.m].name);
#endif
		else if( map[md->bl.m].flag.test_emperium ) {
#ifdef RENEWAL
			status->max_hp += 50 * 100;
			status->max_sp += 70 * 100;
#else
 ...
 * @return guild ID
 */
int status_get_guild_id(struct block_list *bl, bool flag)
{
	nullpo_ret(bl);
	...
	if( !flag && !ranked_getbl_disable(bl) && ranked_show_map(bl->m) && (ranked_getbl_status(bl) == RANK_SHOW || (!bg_team_get_id(bl) && !ranked_get_guild_id(bl))) )
	{
		struct ranked_data *rnk;
		int rank_id = ranked_getbl_rank(bl);

		if( rank_id && (rnk = ranked_search(rank_id)) != NULL && rnk->g != NULL )
			return rnk->g->guild_id;
	}
#endif
 ...
 */
int status_get_emblem_id(struct block_list *bl, bool flag)
{
	nullpo_ret(bl);
